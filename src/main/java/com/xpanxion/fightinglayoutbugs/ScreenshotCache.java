...
import static com.xpanxion.fightinglayoutbugs.ScreenshotCache.Condition.UNMODIFIED;
import static com.xpanxion.fightinglayoutbugs.ScreenshotCache.Condition.WITH_ALL_TEXT_TRANSPARENT;
...
public class ScreenshotCache {
    ...
    void hideImages() {
        if (!_imageUrlsBackedUp) {
            _webPage.executeJavaScript(
                ...
                "}).length;" // ... the trailing ".length" will reduce the size of the response
            );
    ...
    void colorAllText(@Nonnull String color) {
        if (!_textColorsBackedUp) {
            _webPage.executeJavaScript("jQuery('*').each(function() { var $x = jQuery(this); $x.data('flb_color_backup', $x.css('color')); }).length;"); // ... the trailing ".length" will reduce the size of the response
            _textColorsBackedUp = true;
        }
        _webPage.executeJavaScript("jQuery('*').css('color', '" + color + "').length;"); // ... the trailing ".length" will reduce the size of the response
    }

    void restoreTextColors() {
        _webPage.executeJavaScript("jQuery('*').each(function() { var $x = jQuery(this); $x.css('color', $x.data('flb_color_backup')); }).length;"); // ... the trailing ".length" will reduce the size of the response
    }
